...
{
	int first, second;
	cout << "Enter position of 1-st qualified and last worker(existing 0 - " << workers << ") - ";
	do
	{
	...
	{
		cout << "Worker#" << i << ": previous qualif - " << hand[i].get_qualification();
        ++hand[i]; 
	 cout << ", current - " << hand[i].get_qualification() << endl;
	}
