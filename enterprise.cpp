...
void enterprise::input_departments(ifstream &f)
{
  ...
  for (int i = 0; i < num_depart; i++)
  {
	 ...
	 // cout << depart_name << number_work << numberlines << techs;
...
void enterprise::describe_products(int number)
{
	...
	cout << "\nNow you need to describe production of your enterprise\n";
	...
	{    
		...
		cin.clear();
		cout << "Enter list of details(write 'q' and press 'Enter' to stop inputting) - "; istream_iterator<string> iit(cin);
		while (*iit != "q")
		{
			s.push_back(*iit);
			iit++;
	...
	cout << "Enter number details for your list(notice: enter 'q' and press enter after inputting) - "; 
	...
	int count_depart = 0;
	vector<int> r;
	cout << "Enter list of departments that make your product,for example {try : 1 2 4}(notice: enter 'q' and press enter after inputting) - ";
